...
class TestController(TestCase):
    def setUp(self):
        self.mock_detector = MagicMock(autospec='home_greeter.Detector')
        self.mock_greeter = MagicMock(autospec='home_greeter.Greeter')
        self.mock_tweeter = MagicMock(autospec='home_greeter.Tweeter')
        self.controller = Controller(
            detector=self.mock_detector, greeter=self.mock_greeter,
            tweeter=self.mock_tweeter, should_run=False)
    ...
    def test_run(self):
        self.controller.run()
        self.mock_detector.assert_has_calls([call.subscribe(self.controller.process)])
